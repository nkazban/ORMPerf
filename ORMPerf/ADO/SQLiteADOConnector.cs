        ...
        public void AddRandomRowsOneByOne(int count)
        {
            using (var conn = new SqliteConnection(_db.ConnectionString))
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    //var p1 = new SqliteParameter("@Id", Guid.NewGuid().ToString());
                    //command.Parameters.Add(p1);

                    var p2 = new SqliteParameter("@Name", $"{i}");
                    command.Parameters.Add(p2);

                    var p3 = new SqliteParameter("@Birth", DateTime.Now);
                    command.Parameters.Add(p3);

                    var p4 = new SqliteParameter("@About", "");
