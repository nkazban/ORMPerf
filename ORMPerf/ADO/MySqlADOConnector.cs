        ...
        public void AddRandomRowsOneByOne(int count)
        {
            using(var conn = new MySqlConnection(_db.ConnectionString))
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    //var p1 = new MySqlParameter("@Id", Guid.NewGuid().ToByteArray());
                    //command.Parameters.Add(p1);

                    var p2 = new MySqlParameter("@Name", $"{i}");
                    command.Parameters.Add(p2);

                    var p3 = new MySqlParameter("@Birth", DateTime.Now);
                    command.Parameters.Add(p3);

                    var p4 = new MySqlParameter("@About", "");
